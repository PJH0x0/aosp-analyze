...
## [installStage()](https://github.com/TeenagerPeng/aosp-analyze/blob/main/android-R/pkms/install-package/add-package-to-system/code/PackageManagerService.java#L1)
...
## [HandlerParams.startCopy()](https://github.com/TeenagerPeng/aosp-analyze/blob/main/android-R/pkms/install-package/add-package-to-system/code/PackageManagerService.java#L59)
...
## [InstallParams.handleStartCopy()](https://github.com/TeenagerPeng/aosp-analyze/blob/main/android-R/pkms/install-package/add-package-to-system/code/PackageManagerService.java#L251)
...
## [InstallParams.handleReturnCode()](https://github.com/TeenagerPeng/aosp-analyze/blob/main/android-R/pkms/install-package/add-package-to-system/code/PackageManagerService.java#L708)
...
## [processPendingInstall()](https://github.com/TeenagerPeng/aosp-analyze/blob/main/android-R/pkms/install-package/add-package-to-system/code/PackageManagerService.java#L744)

直接调用`processInstallRequestsAsync()`

## [processInstallRequestsAsync()](https://github.com/TeenagerPeng/aosp-analyze/blob/main/android-R/pkms/install-package/add-package-to-system/code/PackageManagerService.java#L755)
...
## [installPackagesTracedLI()](https://github.com/TeenagerPeng/aosp-analyze/blob/main/android-R/pkms/install-package/add-package-to-system/code/PackageManagerService.java#L777)

直接调用`installPackagesLI()`

## [installPackagesLI()](https://github.com/TeenagerPeng/aosp-analyze/blob/main/android-R/pkms/install-package/add-package-to-system/code/PackageManagerService.java#L785)
...
## [preparePackageLI()](https://github.com/TeenagerPeng/aosp-analyze/blob/main/android-R/pkms/install-package/add-package-to-system/code/PackageManagerService.java#L927)
...
详细的逻辑见[APK 签名方案 v2 验证](https://source.android.google.cn/security/apksigning/v2?hl=zh-cn#v2-verification)，这个是签名的官方逻辑，具体的代码逻辑看了一遍和这个差不多，就不进行详细分析了

### [FileInstallArgs.doRename()](https://github.com/TeenagerPeng/aosp-analyze/blob/main/android-R/pkms/install-package/add-package-to-system/code/PackageManagerService.java#L1625)
...
## [optimisticallyRegisterAppId()](https://github.com/TeenagerPeng/aosp-analyze/blob/main/android-R/pkms/install-package/add-package-to-system/code/PackageManagerService.java#L1734)

使用这个方法为应用申请uid，之后一路调用到[acquireAndRegisterNewAppIdLPw()](https://github.com/TeenagerPeng/aosp-analyze/blob/main/android-R/pkms/install-package/add-package-to-system/code/Settings.java#L4304)，所有的uid都放在一个mAppIds的ArrayList下面，分配的规则如下：

1. 看一下mAppIds中有没有没被占用的(有应用被卸载了就会释放出来了)，可以尽量的让空间紧凑，因为安装应用最大也就10000个
2. 如果都占满了，新增一个

## [reconcilePackagesLocked()](https://github.com/TeenagerPeng/aosp-analyze/blob/main/android-R/pkms/install-package/add-package-to-system/code/PackageManagerService.java#L1744)
...
## [commitPackagesLocked()](https://github.com/TeenagerPeng/aosp-analyze/blob/main/android-R/pkms/install-package/add-package-to-system/code/PackageManagerService.java#L1972)
...
### [updateSettingsLI()](https://github.com/TeenagerPeng/aosp-analyze/blob/main/android-R/pkms/install-package/add-package-to-system/code/PackageManagerService.java#L2078)

直接调用的`updateSettingsInternalLI()`

### [updateSettingsInternalLI()](https://github.com/TeenagerPeng/aosp-analyze/blob/main/android-R/pkms/install-package/add-package-to-system/code/PackageManagerService.java#L2083)
...
2. 如果覆盖安装的系统应用，则需要更新一些状态，系统应用如果没有特殊情况，都是为所有用户安装的
   ...
   3. 重置应用的图标
3. 更新overlay的动态库信息
4. 调用[Settings.writeLPr()](https://github.com/TeenagerPeng/aosp-analyze/blob/main/android-R/pkms/install-package/add-package-to-system/code/Settings.java#L2436)更新应用信息到xml中，具体的位置在`/data/system/packages.xml`，这里有个比较有趣的地方是，每一次安装应用都是全写，而不是增量更新，是因为应用数量不多还是为什么不太清楚

## [executePostCommitSteps()](https://github.com/TeenagerPeng/aosp-analyze/blob/main/android-R/pkms/install-package/add-package-to-system/code/PackageManagerService.java#L2223)

这部分的操作基本都与C++层Installer相关了

1. 调用`prepareAppDataAfterInstallLIF()`创建应用内部存储，比如sharedPreference, sqlite db文件就是存在这个目录当中，开发者应当比较熟悉这块的目录
...
1. 签名的源码，原理我知道的差不多，但是从原理转换到源码的过程还是差的远，这里列举一下签名理论的几个文档
...
2. Permission这块的东西没研究很深，但是应该不算复杂，因为PermissionManagerService算是PkMS的一个子模块，另外和权限相关的还有AppOpsManager。这里列举一下相关的文档供查询
   1. [全方位理解Android权限之Android权限系统1](http://wossoneri.github.io/2019/11/17/[Android][Framework]Android-permission-2/)
   2. [Permissions.md](https://github.com/TeenagerPeng/aosp-analyze/blob/main/android-R/pkms/install-package/add-package-to-system/code/Permissions.md)
   3. [AppOps.md](https://github.com/TeenagerPeng/aosp-analyze/blob/main/android-R/pkms/install-package/add-package-to-system/code/AppOps.md)
